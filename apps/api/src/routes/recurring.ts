...
export function formatRecurringResponse(recurring: any): RecurringTransactionResponse {
...
export const recurringRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{ Params: { id: string } }>('/recurring/:id', async (request, reply) => {
  ...
  fastify.patch<{ Params: { id: string } }>('/recurring/:id', async (request, reply) => {
    const { id } = request.params;
    const userId = request.user.id;

    const parseResult = UpdateRecurringTransactionSchema.safeParse(request.body);
    if (!parseResult.success) {
      return reply.status(400).send({
        statusCode: 400,
        error: 'Bad Request',
    ...
    if (payload.frequency !== undefined) data.frequency = payload.frequency as any;
    ...
    if (payload.nextRunDate !== undefined) data.nextRunDate = new Date(payload.nextRunDate);
    if (payload.endDate !== undefined) data.endDate = payload.endDate ? new Date(payload.endDate) : null;
    if (payload.isActive !== undefined) data.isActive = payload.isActive;
    if (payload.isSubscription !== undefined) data.isSubscription = payload.isSubscription;
  ...
  fastify.delete<{ Params: { id: string } }>('/recurring/:id', async (request, reply) => {
    ...
    return reply.send({
      success: true,
      message: 'Recurring template deleted successfully. Historical transactions remain intact.',
    });
  });
