  ...
  fastify.get("/fx/rates", async (request, reply) => {
    ...
    if (!parseResult.success) {
      return reply.status(400).send({
        statusCode: 400,
        error: "Bad Request",
        message: parseResult.error.errors[0]?.message || "Invalid FX query parameters",
      });
    }

    const { baseCurrency, quoteCurrency, date } = parseResult.data;
    const rateInfo = await fxService.getExchangeRate(baseCurrency, quoteCurrency, date);
  ...
  fastify.post("/fx/reporting-currency", async (request, reply) => {
    ...
    if (!parseResult.success) {
      return reply.status(400).send({
        statusCode: 400,
        error: "Bad Request",
        message: parseResult.error.errors[0]?.message || "Invalid reporting currency input",
      });
    }
