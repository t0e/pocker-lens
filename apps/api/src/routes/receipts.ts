...
export function formatExtractionResponse(extraction: any): ReceiptExtractionResponse {
  const items: ReceiptItemResponse[] = (extraction.items || []).map((item: any) => ({
    id: item.id,
    ...
    unitPrice: item.unitPrice !== null && item.unitPrice !== undefined ? parseFloat(item.unitPrice.toString()) : null,
    totalPrice: item.totalPrice !== null && item.totalPrice !== undefined ? parseFloat(item.totalPrice.toString()) : null,
  ...
  return {
    ...
    transactionDate: extraction.transactionDate
      ...
      : null,
    totalAmount: extraction.totalAmount !== null && extraction.totalAmount !== undefined
    ...
    createdAt: extraction.createdAt instanceof Date ? extraction.createdAt.toISOString() : extraction.createdAt,
    updatedAt: extraction.updatedAt instanceof Date ? extraction.updatedAt.toISOString() : extraction.updatedAt,
    items,
    suggestedCategoryName: extraction.category ? extraction.category.name : undefined,
    suggestedAccountName: extraction.account ? extraction.account.name : undefined,
  };
}
...
export function formatReceiptResponse(receipt: any): ReceiptResponse {
  return {
    ...
    extraction: receipt.extraction ? formatExtractionResponse(receipt.extraction) : null,
  };
}
...
export const receiptRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{ Params: { id: string } }>('/receipts/:id/extraction', async (request, reply) => {
    const { id } = request.params;
    const userId = request.user.id;
  ...
  fastify.post<{ Params: { id: string } }>('/receipts/:id/confirm', async (request, reply) => {
    const { id } = request.params;
    const userId = request.user.id;
    ...
    if (receipt.transactionId) {
      return reply.status(400).send({
        statusCode: 400,
        error: 'Bad Request',
        message: 'Receipt has already been confirmed and linked to a transaction',
      });
    }
    ...
    const { createdTx, updatedReceipt } = await prisma.$transaction(async (tx) => {
      // Balance adjustment
  ...
  fastify.post<{ Params: { id: string } }>('/receipts/:id/reprocess', async (request, reply) => {
    const { id } = request.params;
    const userId = request.user.id;
    ...
    if (receipt.transactionId) {
      return reply.status(400).send({
        statusCode: 400,
        error: 'Bad Request',
        message: 'Cannot reprocess receipt that is already linked to a confirmed transaction',
      });
    }
