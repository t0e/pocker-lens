...
describe("Analytics Endpoints (/analytics - Phase 8)", () => {
  ...
  it("GET /analytics/summary returns monthly summary with MoM comparison and savings rate", async () => {
    ...
    if (res.statusCode !== 200) console.log("SUBS ERROR:", res.body); expect(res.statusCode).toBe(200);
  ...
  it("GET /analytics/subscriptions returns normalized subscription costs and commitments", async () => {
    ...
    vi.spyOn(prisma.recurringTransaction as any, "findMany").mockResolvedValueOnce([
      {
        id: "rec_netflix",
        userId: "user_analytics_A",
        type: "EXPENSE",
    ...
    vi.spyOn(prisma.transaction as any, "findMany").mockResolvedValueOnce([
      {
        ...
        createdAt: new Date(),
        updatedAt: new Date(),
        account: { id: "acc_bank", userId: "user_analytics_A", name: "Bank Account", type: "BANK", currency: "VND", currentBalance: new Prisma.Decimal(10000000), openingBalance: new Prisma.Decimal(10000000), isDefault: true, isArchived: false, createdAt: new Date(), updatedAt: new Date() },
        transferAccount: null,
        category: { id: "cat_food", userId: "user_analytics_A", name: "Food", type: "EXPENSE", icon: "utensils", isSystem: true, isArchived: false, createdAt: new Date(), updatedAt: new Date() },
      },
    ] as any);
