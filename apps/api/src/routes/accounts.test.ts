...
describe('Accounts Endpoints (/accounts/*)', () => {
  ...
  describe('POST /accounts', () => {
    it('creates an account with decimal opening balance without floating point loss', async () => {
      ...
      expect(new Prisma.Decimal(body.openingBalance).equals(new Prisma.Decimal(testBalance))).toBe(true);
      expect(new Prisma.Decimal(body.currentBalance).equals(new Prisma.Decimal(testBalance))).toBe(true);
  ...
  describe('GET /accounts', () => {
    it('lists only non-archived accounts belonging to authenticated user by default', async () => {
      ...
      const findManySpy = vi.spyOn(prisma.account, 'findMany').mockResolvedValue(mockAccounts as any);
  ...
  describe('Money Precision Verification', () => {
    const precisionValues = ['0.01', '1000.10', '999999999.99', '0.0001', '1234567.8901'];
    ...
    precisionValues.forEach((amount) => {
      it(`accurately handles precision value ${amount} without floating point error`, async () => {
        ...
        expect(res.statusCode).toBe(200);
        const body = JSON.parse(res.body);
        expect(new Prisma.Decimal(body.openingBalance).equals(decimalVal)).toBe(true);
        expect(new Prisma.Decimal(body.currentBalance).equals(decimalVal)).toBe(true);
      });
    });
  });
});
