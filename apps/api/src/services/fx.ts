...
export class FXService {
  ...
  public async ensureRatesForDate(dateStr: string): Promise<void> {
    ...
    try {
      ...
      const dataToInsert = Object.entries(rates).map(([quoteCurrency, rateNum]) => ({
        baseCurrency: "USD",
      ...
      const dataToInsert = Object.entries(FIXTURE_USD_RATES).map(([quoteCurrency, rateNum]) => ({
        baseCurrency: "USD",
        quoteCurrency: quoteCurrency.toUpperCase(),
        rate: new Prisma.Decimal(rateNum),
        rateDate,
  ...
  public async getExchangeRate(
    ...
    if (from === "USD") {
      ...
      if (row) {
        return { rate: row.rate.toNumber(), rateDate: this.formatDateKey(row.rateDate) };
      }
    }
    ...
    if (to === "USD") {
      ...
      if (row && row.rate.toNumber() > 0) {
        return { rate: 1 / row.rate.toNumber(), rateDate: this.formatDateKey(row.rateDate) };
      }
    }
