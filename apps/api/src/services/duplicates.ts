  ...
  public async checkDuplicates(
    ...
    const candidates = await prisma.transaction.findMany({
      where: {
        ...
        ...(input.excludeTransactionId ? { id: { not: input.excludeTransactionId } } : {}),
      },
      include: {
    ...
    for (const tx of candidates) {
      ...
      const isSameCalendarDay =
        tx.transactionDate.toISOString().slice(0, 10) === candidateDate.toISOString().slice(0, 10);
      
      const txNormMerchant = normalizeMerchant(tx.merchant);
      const txNormDesc = tx.description.toLowerCase().trim();

      const isSameMerchant = normMerchant && txNormMerchant && normMerchant === txNormMerchant;
      const isSameDescription = normDesc && txNormDesc && normDesc === txNormDesc;

      let confidence: DuplicateConfidence;
      let score: number;
      let reason: string;

      if (isSameAccount && isSameCalendarDay && (isSameMerchant || isSameDescription)) {
        confidence = "EXACT";
        score = 1.0;
