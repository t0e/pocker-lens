...
export async function getFinancialSummary(
  ...
  const daysTotal = new Date(Date.UTC(startYear, startMonth + 1, 0)).getUTCDate();
  const isCurrentPeriod =
    now.getUTCFullYear() === startYear && now.getUTCMonth() === startMonth;
  const daysElapsed = isCurrentPeriod ? Math.min(now.getUTCDate(), daysTotal) : daysTotal;
  ...
  for (const currency of Array.from(currenciesSet).sort()) {
    const curExp = currentGroups.find((g) => g.currency === currency && g.type === "EXPENSE");
    const curInc = currentGroups.find((g) => g.currency === currency && g.type === "INCOME");
    ...
    const savingsRate = income > 0 ? Math.round(((income - expenses) / income) * 1000) / 10 : null;

    let momComparison: CurrencyFinancialSummary["momComparison"] = null;
    if (bounds.previous) {
      const prevExp = prevGroups.find((g) => g.currency === currency && g.type === "EXPENSE");
      const prevInc = prevGroups.find((g) => g.currency === currency && g.type === "INCOME");
      ...
      const expenseChangePercentage =
        previousExpenses > 0
          ? Math.round(((expenses - previousExpenses) / previousExpenses) * 1000) / 10
          : null;
...
export async function getCashFlowTrends(
  ...
  monthKeys.forEach((m) => {
    monthBuckets[m.key] = { income: 0, expenses: 0, incomeCount: 0, expenseCount: 0 };
  });

  for (const tx of rawTransactions) {
    const txYear = tx.transactionDate.getUTCFullYear();
    const txMonth = String(tx.transactionDate.getUTCMonth() + 1).padStart(2, "0");
  ...
  const months: MonthlyTrendPoint[] = monthKeys.map(({ key, label }) => {
    const bucket = monthBuckets[key] || { income: 0, expenses: 0, incomeCount: 0, expenseCount: 0 };
    const net = bucket.income - bucket.expenses;
    const savingsRate =
      bucket.income > 0 ? Math.round(((bucket.income - bucket.expenses) / bucket.income) * 1000) / 10 : null;
...
export async function getMerchantBreakdown(
  ...
  const filteredMerchants: { merchant: string; amount: number; count: number }[] = [];
  ...
  const merchants: MerchantSpendingItem[] = topMerchants.map((m) => ({
    ...
    amount: m.amount,
    percentage:
      totalMerchantExpenses > 0 ? Math.round((m.amount / totalMerchantExpenses) * 1000) / 10 : 0,
    transactionCount: m.count,
    averagePerTransaction: m.count > 0 ? Math.round((m.amount / m.count) * 100) / 100 : 0,
  }));
...
export async function getAccountActivity(
  ...
  const activityMap = new Map<
    string,
    { income: number; expenses: number; transfersIn: number; transfersOut: number; count: number }
  >();

  accounts.forEach((a) => {
    activityMap.set(a.id, { income: 0, expenses: 0, transfersIn: 0, transfersOut: 0, count: 0 });
  });
  ...
  const accountItems: AccountActivityItem[] = accounts.map((a) => {
    ...
    const netMovement = act.income - act.expenses + act.transfersIn - act.transfersOut;
...
export async function getBudgetPerformance(
  ...
  const daysElapsed = isCurrentMonth ? Math.min(now.getUTCDate(), daysInMonth) : daysInMonth;
  ...
  const items: BudgetPerformanceItem[] = budgets.map((b) => {
    ...
    const percentageUsed =
      budgetAmount > 0 ? Math.round((actualSpent / budgetAmount) * 1000) / 10 : 0;
    ...
    const expectedPaceAmount =
      daysInMonth > 0 ? Math.round(budgetAmount * (daysElapsed / daysInMonth)) : budgetAmount;
    const pacePercentageDiff =
      expectedPaceAmount > 0
        ? Math.round(((actualSpent - expectedPaceAmount) / expectedPaceAmount) * 1000) / 10
        : 0;
...
export async function generateSpendingInsights(
  ...
  const now = new Date();
  const bounds = getAnalyticsPeriodBounds("current_month", monthStr, undefined, undefined, now);
  ...
  for (const cat of catBreakdown.categories) {
    if (cat.momChangePercentage !== null && Math.abs(cat.momChangeAmount) >= significanceMinAmount) {
  ...
  for (const b of budgetPerf.items) {
    ...
    } else if (b.pacePercentageDiff >= 25 && daysElapsed >= 7 && b.actualSpent < b.budgetAmount) {
      insights.push({
  ...
  if (
    ...
    const subShare = Math.round((commitments.estimatedMonthlyCost / catBreakdown.totalExpenses) * 100);
    insights.push({
