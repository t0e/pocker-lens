...
describe('Receipt Background Worker Processor (processReceiptJob Phase 6)', () => {
  const mockUser = {
    ...
    accounts: [{ id: 'acc_1', name: 'Cash', currency: 'VND', isDefault: true, isArchived: false }],
  };
...
TỔNG CỘNG         80.000 VNĐ
  ...
  it('re-processes receipt by deleting old extraction and creating new extraction cleanly', async () => {
    ...
    vi.spyOn(prisma.receipt, 'findUnique').mockResolvedValue(existingReceipt as any);
    const updateSpy = vi.spyOn(prisma.receipt, 'update').mockResolvedValue({
      ...existingReceipt,
      status: 'READY',
    } as any);
    ...
    const mockTx = {
      receiptExtraction: { create: mockCreateExtraction, delete: mockDeleteExtraction },
    ...
    expect(result.success).toBe(true);
    expect(mockDeleteItems).toHaveBeenCalledWith({ where: { extractionId: 'old_ext_1' } });
    expect(mockDeleteExtraction).toHaveBeenCalledWith({ where: { id: 'old_ext_1' } });
  ...
  it('marks receipt as FAILED when OCR provider throws an error', async () => {
    ...
    await expect(processReceiptJob(mockJob)).rejects.toThrow('Corrupted image data');
