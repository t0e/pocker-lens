...
export async function enhanceContrast(buffer: Buffer): Promise<Buffer> {
  return sharp(buffer).normalize().png({ compressionLevel: 1 }).toBuffer()
}
...
export async function preprocessForOCR(buffer: Buffer): Promise<{
  ...
  return {
    generateCandidates: () =>
      generateCandidateSequence(primaryWorkingBuffer, isCropped, quality),
    debug,
    normalized,
    quality,
  }
