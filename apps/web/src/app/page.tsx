...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [
        ...
        apiClient<MonthlyBudgetsResponse>(
          `/budgets?month=${currentMonth}`,
        ).catch(() => null),
        apiClient<{ upcoming: UpcomingOccurrenceResponse[] }>(
          '/recurring/upcoming?days=14',
        ).catch(() => ({ upcoming: [] })),
        apiClient<{ reportingCurrency: string }>(
          '/fx/reporting-currency',
        ).catch(() => ({ reportingCurrency: 'VND' })),
      ])
  ...
  const totalNetWorth = Object.entries(currencyTotals).reduce(
    (sum, [cur, amt]) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 shadow-sm gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-zinc-500 mt-0.5">
            Multi-currency reporting, exchange rates, smart categorization,
            duplicate protection, and receipt OCR.
          </p>
        </div>
          ...
          {activeAccounts.length === 0 ? (
            <div className="py-4 space-y-2">
              ...
              <p className="text-xs text-zinc-400">
                Create your first cash wallet, bank account, or card to start
                tracking balances.
              </p>
            </div>
          ) : (
              ...
              <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  ...
                  <div className="text-2xl sm:text-3xl font-black text-emerald-400 mt-0.5">
                    {showBalance
                      ? `≈ ${formatCurrencyAmount(totalNetWorth, reportingCurrency)}`
                      : '••••••••'}
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-4 sm:p-5 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            {monthlyBudgets &&
            Object.keys(monthlyBudgets.summaries).length > 0 ? (
              <div className="space-y-3 pt-1">
                {Object.entries(monthlyBudgets.summaries).map(
                  ([currency, sum]) => (
                    <div
                      key={currency}
                      className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-950 border border-zinc-200/80 dark:border-zinc-800 space-y-2"
                    >
                      ...
                      <div className="flex items-center justify-between text-[11px] text-zinc-400">
                        <span>
                          {formatMoney(sum.totalSpent, currency)} spent
                        </span>
                        <span>
