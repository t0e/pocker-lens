...
export const DataQualityCard: React.FC<DataQualityCardProps> = ({
  ...
  const totalIssues =
    ...
    report.potentialDuplicatesCount +
    report.pendingReceiptsCount
  if (totalIssues === 0) {
    return (
      <div
        className={`bg-slate-900/60 border border-emerald-500/20 rounded-2xl p-4 flex items-center justify-between ${className}`}
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-xs font-semibold text-white">
              Financial Data Quality: 100%
            </h4>
            <p className="text-[11px] text-slate-400">
              All transactions categorized, 0 duplicates detected.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div
      className={`bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3.5 ${className}`}
    >
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h4 className="text-xs font-bold text-white">
              Data Quality & Smart Hygiene
            </h4>
            <p className="text-[11px] text-slate-400">
              {totalIssues} items need your attention
            </p>
          </div>
        </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (onCategorizeClick) {
                        onCategorizeClick(
                          tx.id,
                          tx.suggestedCategory!.categoryId!,
                        )
                      } else {
