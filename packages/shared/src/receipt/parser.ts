...
const GRAND_TOTAL_REGEX =
  /(?:^|\s)(?:TỔNG\s*CỘNG|TONG\s*CONG|GRAND\s*TOTAL|TOTAL\s*DUE|AMOUNT\s*DUE|TỔNG\s*THANH\s*TOÁN|TONG\s*THANH\s*TOAN|BALANCE\s*DUE|TOTAL\s*PAID|PAYMENT\s*TOTAL|\bTOTAL\b|\bAMOUNT\b|\bSỐ\s*TIỀN\b|\bSO\s*TIEN\b)(?!\s*item|\s*qty|\s*unit|\s*món|\s*sp|\s*mặt\s*hàng)/i
const PURCHASE_VALUE_REGEX =
  /(?:^|\s)(?:GIÁ\s*TRỊ\s*MUA|GIA\s*TRI\s*MUA|THÀNH\s*TIỀN|THANH\s*TIEN|TỔNG\s*TIỀN|TONG\s*TIEN|NET\s*TOTAL|FINAL\s*AMOUNT)(?!\s*item|\s*qty|\s*unit|\s*món|\s*sp|\s*mặt\s*hàng)/i
const PAYMENT_KEYWORDS_REGEX =
  /(?:^|\s|\+)(?:MOMO|VNPAY|ZALOPAY|THANH\s*TOÁN\s*QR|CHUYỂN\s*KHOẢN|CARD\b|VISA\b|MASTERCARD|TIỀN\s*KHÁCH\s*TRẢ|TIEN\s*KHACH\s*TRA|THANH\s*TOÁN|THANH\s*TOAN|BANK\b|AGRI\b|VCB\b|VTB\b|TPB\b|MB\b|ACB\b|SHB\b|HDB\b|OCB\b|EXIMBANK|VIETIN\b|TECHCOMBANK|SACOMBANK)/i
const TOTAL_ITEMS_COUNT_REGEX =
  /(?:TỔNG\s*SỐ(?:\s*LƯỢNG|\s*MÓN|\s*SP|\s*MẶT\s*HÀNG)?|TONG\s*SO(?:\s*LUONG|\s*MON|\s*SP)?|\bTOTAL\s*ITEMS\b|\bTOTAL\s*QTY\b|\bQTY\s*TOTAL\b|\bITEMS?\s*COUNT\b)/i
const CASH_GIVEN_REGEX =
  /(?:^|\s)(?:TIỀN\s*KHÁCH\s*ĐƯA|TIEN\s*KHACH\s*DUA|TIỀN\s*MẶT|TIEN\s*MAT|CASH\s*TENDERED|CASH\s*RECEIVED|\bCASH\b|\bGIVEN\b)/i
const CHANGE_REGEX =
  /(?:^|\s)(?:TIỀN\s*THỐI|TIEN\s*THOI|TIỀN\s*THỪA|TIEN\s*THUA|TIỀN\s*TRẢ\s*LẠI|TIEN\s*TRA\s*LAI|\bCHANGE\b)/i
const SUBTOTAL_REGEX =
  /(?:^|\s)(?:SUBTOTAL|SUB\s*TOTAL|TIỀN\s*HÀNG|TIEN\s*HANG|TỔNG\s*TIỀN\s*HÀNG|TẠM\s*TÍNH|TAM\s*TINH)/i
const DISCOUNT_REGEX =
  /(?:^|\s)(?:DISCOUNT|GIẢM\s*GIÁ|GIAM\s*GIA|CHIẾT\s*KHẤU|CHIET\s*KHAU|\bKM\b)/i
const TAX_REGEX = /(?:^|\s)(?:TAX\b|VAT\b|THUẾ|THUE)/i
const ITEM_BREAKDOWN_REGEX =
  /^(?:unit\s*price|line\s*total|item\s*total|đơn\s*giá|don\s*gia|giá\s*gốc|gia\s*goc|qty\b|sl\b)/i
...
export function isFormattedMonetary(rawStr: string): boolean {
  return (
    /^\d{1,3}(?:[.,]\d{3})+$/.test(rawStr) ||
    /^\d{1,3}(?:,\d{3})*\.\d{2}$/.test(rawStr) ||
    /^\d{1,3}(?:\.\d{3})*,\d{2}$/.test(rawStr)
  )
}
 ...
 */
export function extractDate(lines: string[]): {
  date: Date | null
  ...
  for (const line of lines) {
    ...
    const vnFullDate = line.match(
      /(?:ngày|ngay)\s+(\d{1,2})\s+(?:tháng|thang|th)\s+(\d{1,2})\s+(?:năm|nam)\s+(20\d{2})/i,
    )
    if (vnFullDate) {
...
export function extractLineItems(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (amounts && amounts.length > 0) {
      ...
      if (alphaCount < 2 || desc.includes('...') || desc.includes('___'))
        continue
      ...
      if (
        ...
        const qtyMatch =
          prefix.match(/\b([1-9]\d*)\s*[xX]\b/) ||
          prefix.match(/\b[xX]\s*([1-9]\d*)\b/)
        if (qtyMatch) {
        ...
        if (amounts.length >= 2) {
          ...
          if (
            possibleUnitPrice !== null &&
            possibleUnitPrice > 0 &&
            possibleUnitPrice !== parsedTotalPrice
          ) {
...
function applyArithmeticCrossValidation(
  ...
  if (
    ...
    const netCalculated =
      subtotalAmount + (taxAmount || 0) - (discountAmount || 0)
    if (netCalculated > 0) {
      for (const c of candidates) {
        if (
          Math.abs(c.amount - netCalculated) < 0.01 &&
          c.role !== 'subtotal'
        ) {
          c.score += 45
          hasArithmeticProof = true
          c.reasons.push(
            `Subtotal + Tax - Discount arithmetic match (${subtotalAmount} + ${taxAmount || 0} - ${discountAmount || 0} = ${netCalculated}) (+45)`,
          )
        }
      }
    }
  ...
  if (cashGivenAmount !== null && changeAmount !== null) {
    ...
    if (netPaid > 0) {
      for (const c of candidates) {
        if (
          Math.abs(c.amount - netPaid) < 0.01 &&
          c.role !== 'cash_given' &&
          c.role !== 'change'
        ) {
          c.score += 45
          hasArithmeticProof = true
          c.reasons.push(
            `Cash - Change arithmetic match (${cashGivenAmount} - ${changeAmount} = ${netPaid}) (+45)`,
          )
        }
      }
    }
  ...
  for (const c of validCandidates) {
    for (const other of validCandidates) {
      if (c !== other && areNumericallyConfusable(c.amount, other.amount)) {
        const cHasPayment = validCandidates.some(
          (v) =>
            Math.abs(v.amount - c.amount) < 0.01 && v.role === 'payment_amount',
        )
        const cHasArithmetic =
          lineItemSum > 0 && Math.abs(c.amount - lineItemSum) < 0.01
        const otherHasPayment = validCandidates.some(
          (v) =>
            Math.abs(v.amount - other.amount) < 0.01 &&
            v.role === 'payment_amount',
        )
        const otherHasArithmetic =
          lineItemSum > 0 && Math.abs(other.amount - lineItemSum) < 0.01

        if (
          (cHasPayment || cHasArithmetic) &&
          !otherHasPayment &&
          !otherHasArithmetic
        ) {
          c.score += 35
...
export function extractReceiptData(
  ...
  const hasVnKeywords =
    /\b(hoá đơn|hóa đơn|phiếu|thanh toán|thanh toan|tổng cộng|tong cong|thành tiền|thanh tien|tiền mặt|tien mat|cà phê|ca phe|quán|quan|bán hàng|ban hang|cửa hàng|cua hang|giá trị mua|gia tri mua|siêu thị|sieu thi|fujimart|momo|tiền khách trả|tien khach tra)\b/i.test(
      normalizedText,
    )
  ...
  // 4. Date Detection
  const { date: transactionDate, confidence: dateConfidence } =
    extractDate(lines)
  ...
  const combinedSearchText =
    `${merchant || ''} ${lines.join(' ')}`.toLowerCase()
  ...
  for (const [catName, keywords] of Object.entries(categoryKeywords)) {
    for (const kw of keywords) {
      if (combinedSearchText.includes(kw.toLowerCase())) {
        ...
        categoryConfidence = merchant?.toLowerCase().includes(kw.toLowerCase())
          ? 'high'
          : 'medium'
        break
      }
