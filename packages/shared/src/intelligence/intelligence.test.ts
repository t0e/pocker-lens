...
describe("Merchant Normalization", () => {
  ...
  it("should remove noisy punctuation while keeping alphanumeric tokens", () => {
    expect(normalizeMerchant("Masteri Thao Dien - Landlord #01")).toBe("masteri thao dien landlord 01");
    expect(normalizeMerchant("STARBUCKS (VINCOM MALL)")).toBe("starbucks vincom mall");
  });
