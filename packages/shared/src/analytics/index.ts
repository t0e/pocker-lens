...
export type SpendingPaceStatus = "ON_TRACK" | "AHEAD_OF_PACE" | "BEHIND_PACE" | "OVER_BUDGET";
...
export function getAnalyticsPeriodBounds(
  ...
  now = new Date()
): { current: { start: Date; end: Date; label: string }; previous?: { start: Date; end: Date } } {
  ...
  if (customMonth && /^\d{4}-\d{2}$/.test(customMonth)) {
    ...
    const prevStart = new Date(Date.UTC(m === 1 ? y - 1 : y, m === 1 ? 11 : m - 2, 1, 0, 0, 0, 0));
    const prevEnd = new Date(Date.UTC(m === 1 ? y - 1 : y, m === 1 ? 12 : m - 1, 0, 23, 59, 59, 999));
  ...
  if (timeRange === "previous_month") {
    ...
    const end = new Date(Date.UTC(prevYear, prevMonthIdx + 1, 0, 23, 59, 59, 999));
    const beforeYear = prevMonthIdx === 0 ? prevYear - 1 : prevYear;
    const beforeMonthIdx = prevMonthIdx === 0 ? 11 : prevMonthIdx - 1;
    const prevStart = new Date(Date.UTC(beforeYear, beforeMonthIdx, 1, 0, 0, 0, 0));
    const prevEnd = new Date(Date.UTC(beforeYear, beforeMonthIdx + 1, 0, 23, 59, 59, 999));
    const label = start.toLocaleString("en-US", { month: "long", year: "numeric", timeZone: "UTC" });
    return {
      current: { start, end, label },
      previous: { start: prevStart, end: prevEnd },
  ...
  if (timeRange === "last_3_months") {
    const start = new Date(Date.UTC(currentYear, currentMonthIdx - 2, 1, 0, 0, 0, 0));
    const end = new Date(Date.UTC(currentYear, currentMonthIdx + 1, 0, 23, 59, 59, 999));
    const prevStart = new Date(Date.UTC(currentYear, currentMonthIdx - 5, 1, 0, 0, 0, 0));
    const prevEnd = new Date(Date.UTC(currentYear, currentMonthIdx - 2, 0, 23, 59, 59, 999));
    return {
  ...
  if (timeRange === "last_6_months") {
    const start = new Date(Date.UTC(currentYear, currentMonthIdx - 5, 1, 0, 0, 0, 0));
    const end = new Date(Date.UTC(currentYear, currentMonthIdx + 1, 0, 23, 59, 59, 999));
    const prevStart = new Date(Date.UTC(currentYear, currentMonthIdx - 11, 1, 0, 0, 0, 0));
    const prevEnd = new Date(Date.UTC(currentYear, currentMonthIdx - 5, 0, 23, 59, 59, 999));
    return {
  ...
  const prevEnd = new Date(Date.UTC(prevYear, prevMonthIdx + 1, 0, 23, 59, 59, 999));
  const label = start.toLocaleString("en-US", { month: "long", year: "numeric", timeZone: "UTC" });
